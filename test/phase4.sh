...
q "CREATE TABLE bt (a int, b text) USING columnar;" >/dev/null
...
q "CREATE TABLE ht (a int, b text) USING columnar;" >/dev/null
...
q "CREATE TABLE uq (id int PRIMARY KEY, v text) USING columnar;" >/dev/null
...
q "CREATE TABLE uq2 (id int, v text) USING columnar;" >/dev/null
...
q "CREATE TABLE ck (a int NOT NULL, b int CHECK (b > 0)) USING columnar;" >/dev/null
...
q "CREATE TABLE di (a int, b text) USING columnar;" >/dev/null
...
q "SELECT columnar.alter_table_set_access_method('conv','columnar');" >/dev/null
check "conv now columnar"  "$(q "SELECT amname FROM pg_am m JOIN pg_class c ON c.relam=m.oid WHERE c.relname='conv';")" "columnar"
...
q "SELECT columnar.alter_table_set_access_method('conv','heap');" >/dev/null
...
# an ordinary seq scan is still available when enabled
assert_plan_seq() {
	local plan
	...
	if echo "$plan" | grep -q "Seq Scan"; then
		echo "PASS  seq scan available"
	else
		echo "FAIL  seq scan available: $plan"; fail=1
	fi
}
assert_plan_seq
